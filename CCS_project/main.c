 ...
 * main.c
 *
 * main for solar panel control project
 * contains:
 * constant and global variables initialization
...
#include <Hardware/PowerDriver/HAL_INA219.h>
...
StateMachine_t fsm[] = {
      {UP_STATE, draw_current_menu},
      {DOWN_STATE, draw_sensors_menu},
      {LEFT_STATE, draw_mqtt_menu},
      {RIGHT_STATE, draw_alarm_menu}
};
...
// send each value char by char
void mqtt_sensor_send () {
    // TEMPERATURE
    UART_transmitData(EUSCI_A2_BASE, 'T');
    UART_transmitData(EUSCI_A2_BASE, 'E');
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[0] /10      %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[0] /1       %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[1] /10      %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[1] /1       %10);
    UART_transmitData(EUSCI_A2_BASE, '#');
    // HUMIDITY
    UART_transmitData(EUSCI_A2_BASE, 'H');
    UART_transmitData(EUSCI_A2_BASE, 'U');
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[2] /10      %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[2] /1       %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[3] /10      %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[3] /1       %10);
    UART_transmitData(EUSCI_A2_BASE, '#');
    // LIGHT
    UART_transmitData(EUSCI_A2_BASE, 'L');
    UART_transmitData(EUSCI_A2_BASE, 'U');
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /1000000 %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /100000  %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /10000   %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /1000    %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /100     %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /10      %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[4] /1       %10);
    UART_transmitData(EUSCI_A2_BASE, '#');
    // CURRENT
    UART_transmitData(EUSCI_A2_BASE, 'C');
    UART_transmitData(EUSCI_A2_BASE, 'U');
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[5] /1000    %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[5] /100     %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[5] /10      %10);
    UART_transmitData(EUSCI_A2_BASE, samplingBuffer[5] /1       %10);
    UART_transmitData(EUSCI_A2_BASE, '#');
}
...
void EUSCIA2_IRQHandler(void) {
    ...
    if(status & EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG) {
        ...
        if (RXData != '#') {
            ...
            // [h, day, month, year, curr]
            } else if (strncmp(esp_data, "h", 1) == 0) {
                uint16_t buff[2];
                sscanf(esp_data, "h%1hu%2hu", &buff[0], &buff[1]);
                mqttBuffer[buff[0]][0] = buff[1];
            } else if (strncmp(esp_data, "d", 1) == 0) {
                uint16_t buff[2];
                sscanf(esp_data, "d%1hu%2hu", &buff[0], &buff[1]);
                mqttBuffer[buff[0]][1] = buff[1];
            } else if (strncmp(esp_data, "m", 1) == 0) {
                uint16_t buff[2];
                sscanf(esp_data, "m%1hu%2hu", &buff[0], &buff[1]);
                mqttBuffer[buff[0]][2] = buff[1];
            } else if (strncmp(esp_data, "y", 1) == 0) {
                uint16_t buff[2];
                sscanf(esp_data, "y%1hu%4hu", &buff[0], &buff[1]);
                mqttBuffer[buff[0]][3] = buff[1];
            } else if (strncmp(esp_data, "v", 1) == 0) {
                uint16_t buff[2];
                sscanf(esp_data, "v%1hu%4hu", &buff[0], &buff[1]);
                mqttBuffer[buff[0]][4] = buff[1];

            } else if (strcmp(esp_data, "MQ0") == 0) {      // server MQTT CONNECTED
                current_state_mqtt = CONNECTED;
            } else if (strcmp(esp_data, "MQ1") == 0) {      // server MQTT OFFLINE
                current_state_mqtt = OFFLINE;
            }
...
void TA0_N_IRQHandler(void){
    ...
    samplingBuffer[5] = ina219_getCurrent (); // current

    // to mqtt
    mqtt_sensor_send ();

    if (alarm_active && samplingBuffer[5] < minCurrent (samplingBuffer)) {
        start_alarm ();
    }
    ...
    if(status & ADC_INT1 && !alarm_running) {
        ...
        if (joystickBuffer[0] > 6000 && joystickBuffer[0] < 11000){
            ...
            if (joystickBuffer[0] > 13000 && current_state != RIGHT_STATE) {
                ...
                new_state ();
                // ask mqtt for status CONNECTED/OFFLINE
